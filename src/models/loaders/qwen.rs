// Qwen Loader - Refactored for UnifiedModelLoader
// Loads Qwen 2.5 models (1.5B/3B/7B/14B) on any backend (Metal, CPU, CUDA)
...
/// Qwen generator implementing TextGeneration trait
...
/// Load Qwen model from cache directory with specified device
///
/// # Arguments
...
pub fn load(model_path: &Path, size: ModelSize, device: Device) -> Result<Box<dyn TextGeneration>> {
    ...
    tracing::info!("Loading Qwen {} from {:?} on {:?}", size_str, model_path, device);
    ...
    let config: Qwen2Config = serde_json::from_reader(
    ...
    tracing::info!("Successfully loaded Qwen {}", size_str);

    let name = format!("Qwen 2.5 {}", size_str);

    Ok(Box::new(QwenGenerator {
        model,
        tokenizer,
        config,
        device,
